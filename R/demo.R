...
demo_geneinfo <- function() {
  ...
  geneinfo_demo <- data.frame(
    symbol = gene1, synonyms = gene2, species = species,
    stringsAsFactors = F
  )
...
demo_lrpairs <- function() {
  ...
  species <- c("Human", "Human", "Human", "Mouse")
  lrpairs_demo <- data.frame(
    ligand = ligand, receptor = receptor, species = species,
    stringsAsFactors = F
  )
  return(lrpairs_demo)
}
...
demo_pathways <- function() {
  ...
  pathway <- c(
    "p53 signaling pathway", "p53 signaling pathway", "p53 signaling pathway",
    "PI3K-Akt signaling pathway"
  )
  type <- c(
    "Process(activation)", "Process(activation)", "Process(binding)",
    "Process(association)"
  )
  ...
  species <- c("Human", "Human", "Human", "Mouse")
  pathways_demo <- data.frame(
    src = src, dest = dest, pathway = pathway, source = sourcename,
