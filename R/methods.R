...
dec_celltype <- function(
    object, sc_data, sc_celltype, min_percent = 0.5, min_nFeatures = 10, if_use_normalize_data = T, if_use_hvg = F,
  ...
  # OPTIMIZED: Use more CPU cores by default (75% instead of 25%)
  if (if_doParallel) {
    if (is.null(use_n_cores)) {
      n_cores <- parallel::detectCores()
      n_cores <- max(1, floor(n_cores * 0.75)) # Use 75% of cores
    } else {
      n_cores <- use_n_cores
    }
    n.threads <- n_cores
...
find_lr_path <- function(object, lrpairs, pathways, max_hop = NULL, if_doParallel = T, use_n_cores = NULL) {
  ...
  # OPTIMIZED: Use more CPU cores (75% instead of 25%)
  if (is.null(use_n_cores)) {
    n_cores <- parallel::detectCores()
    n_cores <- max(1, floor(n_cores * 0.75))
    if (n_cores < 2) {
      if_doParallel <- FALSE
    }
  } else {
...
dec_cci <- function(
    object, celltype_sender, celltype_receiver, n_neighbor = 10, min_pairs = 5,
...
dec_cci_all <- function(
    object, n_neighbor = 10, min_pairs = 5, min_pairs_ratio = 0,
  ...
  for (i in 1:length(cellname)) {
    d1 <- data.frame(
      celltype_sender = rep(cellname[i], length(cellname)), celltype_receiver = cellname,
      stringsAsFactors = F
    )
  ...
  cat(crayon::cyan("Begin to find LR pairs", "\n"))
  if (if_doParallel) {
    all_res <- foreach::foreach(
      i = 1:nrow(celltype_pair), .packages = c("Matrix", "reshape2"),
      .export = c(".get_cellpair", ".lr_distance", ".get_tf_res", ".get_score")
    ) %dopar% {
...
get_lr_path <- function(object, celltype_sender, celltype_receiver, ligand, receptor, min_gene_num = 5) {
  ...
  for (j in 1:length(rec_pathway_all)) {
    ...
    if (length(gene_pathway) >= min_gene_num) {
      ...
      a <- matrix(c(
        gene_pathway_yes_num, gene_rec_num - gene_pathway_yes_num,
        gene_pathway_num - gene_pathway_yes_num, gene_all_num - gene_rec_num +
          gene_pathway_yes_num - gene_pathway_num
      ), nrow = 2)
