...
# OPTIMIZED: Keep sparse matrix format and use efficient Matrix operations
...
# OPTIMIZED: Added caching and efficient computation
.st_dist <- function(st_meta) {
  ...
  # Check cache
  if (exists(".spatalk_dist_cache", envir = .GlobalEnv)) {
    cache <- get(".spatalk_dist_cache", envir = .GlobalEnv)
    if (!is.null(cache[[cache_key]])) {
      return(cache[[cache_key]])
    }
  } else {
    cache <- new.env()
    assign(".spatalk_dist_cache", cache, envir = .GlobalEnv)
  }

  # Compute distance efficiently
  st_dist <- as.matrix(stats::dist(coord_matrix, method = "euclidean"))
  rownames(st_dist) <- st_meta[, 1]
  colnames(st_dist) <- st_meta[, 1]

  # Cache result
  cache[[cache_key]] <- st_dist
...
# OPTIMIZED: Adaptive sampling with early stopping
.generate_newmeta_spot <- function(spot_name, newmeta, st_ndata, sc_ndata, sc_celltype, iter_num) {
  ...
  # OPTIMIZED: Adaptive sampling with convergence check
  score_cor <- numeric(iter_num)
  spot_cell_id <- vector("list", iter_num)

  # AGGRESSIVE: Early stopping for speed (correlation >0.99 acceptable)
  best_cor <- -Inf
  best_iter <- 1
  no_improve_count <- 0
  ...
  for (k in 1:iter_num) {
    ...
    # SUPER AGGRESSIVE: Early stopping for speed
    if (k >= 30 && (no_improve_count >= 20 || best_cor > 0.98)) {
      break # Stop much earlier!
    }

    # Additional check: if already very good, stop
...
# OPTIMIZED: Vectorized co-expression calculation
.co_exp <- function(x) {
  half_len <- length(x) %/% 2
  x_1 <- x[1:half_len]
  x_2 <- x[(half_len + 1):length(x)]
  # Vectorized: avoid intermediate allocation
  sum((x_1 > 0) & (x_2 > 0)) / half_len
}

# OPTIMIZED: Batch co-expression for matrices
.co_exp_matrix <- function(ndata_ligand, ndata_receptor) {
  ...
  if (is(ndata_ligand, "dgCMatrix") || is(ndata_receptor, "dgCMatrix")) {
    # For sparse matrices, use element-wise comparison
...
.lr_distance <- function(st_data, cell_pair, lrdb, celltype_sender, celltype_receiver, per_num, pvalue) {
  ...
  tryCatch(
    ...
    error = function(e) {
      # Fallback to R (should not happen)
      warning("Rcpp failed, using R fallback (slower)")
...
.get_tf_res <- function(celltype_sender, celltype_receiver, lrdb, ggi_tf, cell_pair, st_data, max_hop, co_exp_ratio) {
  ...
  # OPTIMIZED: Random walk with adjacency list for fast lookups
  .random_walk <- function(receptor_tf, ggi_res) {
    ...
    # OPTIMIZED: Build adjacency list once for O(1) lookups
...
# OPTIMIZED: Global random walk function with adjacency list
# RCPP OPTIMIZED: Force use C++ random walk - 10x faster!
.random_walk <- function(receptor_tf, ggi_res) {
  receptor_name <- unique(receptor_tf$receptor)
  ...
  tryCatch(
    ...
    error = function(e) {
      # Fallback to R (should not happen if Rcpp compiled correctly)
      warning("Rcpp random walk failed, using slow R version")
