...
.run_seurat <- function(st_data, sc_data, sc_celltype) {
  ...
  predictions.assay <- Seurat::TransferData(
    anchorset = anchors, refdata = ref_data$celltype,
...
# Run stereoscope deconvolution (supports Windows, macOS, Linux)
.run_stereoscope <- function(st_data, sc_data, sc_celltype, env, anaconda_path) {
  ...
  # Build stereoscope command
  stereo_cmd <- "stereoscope run --sc_cnt cnt_data.tsv --sc_labels mta_data.tsv -sce 75000 -o res -n 5000 --st_cnt cnt_st.tsv -ste 75000 --gpu -stb 100 -scb 100"
  
  if (is_windows) {
    # Windows: Use conda run or activate via cmd
    if (env == "base") {
      cmd <- paste0("conda activate && ", stereo_cmd)
    } else {
      cmd <- paste0("conda activate ", env, " && ", stereo_cmd)
    }
    # Execute via cmd.exe
    exit_code <- system(paste0("cmd /c \"", cmd, "\""), wait = TRUE)
  } else {
    # Unix-like systems (macOS, Linux)
    if (env == "base") {
      res_sh <- c(
        "#!/bin/bash",
        paste0("source ", anaconda_path, "/etc/profile.d/conda.sh 2>/dev/null || eval \"$(conda shell.bash hook)\""),
        "conda activate",
        stereo_cmd
      )
    } else {
    ...
    Sys.chmod("res_sh.sh", mode = "755")
    exit_code <- system("./res_sh.sh", wait = TRUE)
  }
  
  if (exit_code != 0) {
    stop("stereoscope execution failed. Please check your conda environment and stereoscope installation.")
  }
  
  filename <- dir("res/cnt_st/")
  if (length(filename) == 0) {
    stop("No results found in res/cnt_st/. Please check stereoscope output.")
  }
  ...
  st_coef <- readr::read_tsv(paste0("res/cnt_st/", filename), show_col_types = FALSE)
...
# Generate reference matrix from single-cell data
.generate_ref <- function(sc_ndata, sc_celltype, if_doParallel) {
  ref_celltype <- unique(sc_celltype$celltype)
  ref_celltype <- sort(ref_celltype) # Sort upfront

  if (if_doParallel) {
    sc_ref <- foreach::foreach(i = 1:length(ref_celltype), .combine = "cbind", .packages = "Matrix") %dopar% {
      cells <- sc_celltype[sc_celltype$celltype == ref_celltype[i], ]$cell
      sc_data_celltype <- sc_ndata[, cells, drop = FALSE]

      # Use Matrix::rowMeans for efficient sparse matrix operations
      if (is(sc_data_celltype, "dgCMatrix")) {
        Matrix::rowMeans(sc_data_celltype)
      } else if (ncol(sc_data_celltype) > 1) {
        rowMeans(as.matrix(sc_data_celltype))
      } else {
    ...
    sc_ref <- matrix(0, nrow = nrow(sc_ndata), ncol = length(ref_celltype))
    rownames(sc_ref) <- rownames(sc_ndata)
    colnames(sc_ref) <- ref_celltype
    ...
    for (i in 1:length(ref_celltype)) {
      ...
      # Use Matrix:: for automatic sparse/dense handling
      if (is(sc_data_celltype, "dgCMatrix")) {
        sc_ref[, i] <- Matrix::rowMeans(sc_data_celltype)
      } else if (ncol(sc_data_celltype) > 1) {
        sc_ref[, i] <- rowMeans(as.matrix(sc_data_celltype))
      } else {
        sc_ref[, i] <- as.numeric(sc_data_celltype)
      }
    }
  }

  return(as.data.frame(sc_ref, stringsAsFactors = FALSE))
}
...
# Normalize coefficient matrix by row sums
.coef_nor <- function(st_coef) {
  row_sums <- rowSums(st_coef)
  # Avoid division by zero
  row_sums[row_sums == 0] <- 1
  st_coef_norm <- sweep(st_coef, 1, row_sums, "/")
  return(as.data.frame(st_coef_norm))
}

# Package-level cache environment
.spatalk_cache_env <- new.env(parent = emptyenv())

# Compute Euclidean distance matrix
.st_dist <- function(st_meta) {
  # Create cache key based on coordinates
  coord_matrix <- cbind(st_meta$x, st_meta$y)
  cache_key <- paste0(
    "dist_", nrow(st_meta), "_",
    digest::digest(coord_matrix, algo = "xxhash64")
  )

  # Check package-level cache (safer than global environment)
  if (exists(cache_key, envir = .spatalk_cache_env)) {
    return(get(cache_key, envir = .spatalk_cache_env))
  }

  # Compute distance efficiently using C++ if available, otherwise R
  st_dist <- tryCatch({
    if (nrow(st_meta) > 0) {
      d <- cpp_fast_dist(st_meta$x, st_meta$y)
      rownames(d) <- st_meta[, 1]
      colnames(d) <- st_meta[, 1]
      d
    } else {
      d <- as.matrix(stats::dist(coord_matrix, method = "euclidean"))
      rownames(d) <- st_meta[, 1]
      colnames(d) <- st_meta[, 1]
      d
    }
  }, error = function(e) {
    # Fallback to R's dist function
    d <- as.matrix(stats::dist(coord_matrix, method = "euclidean"))
    rownames(d) <- st_meta[, 1]
    colnames(d) <- st_meta[, 1]
    d
  ...
  # Cache result in package environment (limited size)
  if (length(ls(.spatalk_cache_env)) < 10) {
    assign(cache_key, st_dist, envir = .spatalk_cache_env)
  }
...
.generate_newmeta <- function(st_meta, st_dist, min_percent) {
  ...
  newmeta <- data.frame(
    spot = newmeta_spot, cell_ratio = newmeta_ratio, celltype = newmeta_cell,
...
# Generate metadata for a single spot with cell sampling
.generate_newmeta_spot <- function(spot_name, newmeta, st_ndata, sc_ndata, sc_celltype, iter_num) {
  ...
  score_cor <- numeric(iter_num)
  spot_cell_id <- vector("list", iter_num)

  # Track best result with early stopping
  best_cor <- -Inf
  best_iter <- 1
  no_improve_count <- 0

  for (k in 1:iter_num) {
    spot_cell_id_k <- character(nrow(newmeta_spot))
    for (j in 1:nrow(newmeta_spot)) {
      ...
      spot_cell_id_k[j] <- sc_celltype1[sample.int(length(sc_celltype1), 1)]
    }
    ...
    if (length(spot_cell_id_k) == 1) {
      ...
      if (is(sc_ndata, "dgCMatrix")) {
        spot_ndata_pred <- as.numeric(Matrix::rowSums(sc_ndata[, spot_cell_id_k]))
      } else {
    ...
    score_cor[k] <- spot_ndata_cor
    spot_cell_id[[k]] <- spot_cell_id_k

    # Track best and early stop
    if (spot_ndata_cor > best_cor) {
      best_cor <- spot_ndata_cor
      best_iter <- k
      no_improve_count <- 0
    } else {
    ...
    if (k >= 30 && (no_improve_count >= 20 || best_cor > 0.98)) {
      break
    }

    if (k >= 20 && best_cor > 0.95 && no_improve_count >= 10) {
      break
    }
  }

  # Return best result
  spot_cell_id <- spot_cell_id[[best_iter]]
  newmeta_spot$cell_id <- spot_cell_id
  newmeta_spot$cor <- best_cor
  return(newmeta_spot)
}
...
# Calculate co-expression ratio for paired vectors
.co_exp <- function(x) {
  n <- length(x)
  if (n < 2) return(0)
  half_len <- n %/% 2
  x_1 <- x[1:half_len]
  x_2 <- x[(half_len + 1):(half_len * 2)]
  if (half_len == 0) return(0)
  sum((x_1 > 0) & (x_2 > 0)) / half_len
}

# Calculate co-expression ratio for matrices
.co_exp_matrix <- function(ndata_ligand, ndata_receptor) {
  if (is(ndata_ligand, "dgCMatrix") || is(ndata_receptor, "dgCMatrix")) {
    lr_both_expressed <- (ndata_ligand > 0) & (ndata_receptor > 0)
    if (is.matrix(lr_both_expressed)) {
      return(rowSums(lr_both_expressed) / ncol(lr_both_expressed))
    } else {
      return(Matrix::rowSums(lr_both_expressed) / ncol(lr_both_expressed))
    }
  } else {
    return(rowSums((ndata_ligand > 0) & (ndata_receptor > 0)) / ncol(ndata_ligand))
  }
}
...
# Calculate LR distance with parallel support
.lr_distance_doParallel <- function(st_data, cell_pair, lrdb, celltype_sender, celltype_receiver, per_num, pvalue) {
  return(.lr_distance(st_data, cell_pair, lrdb, celltype_sender, celltype_receiver, per_num, pvalue))
}

# Calculate LR distance with permutation test
.lr_distance <- function(st_data, cell_pair, lrdb, celltype_sender, celltype_receiver, per_num, pvalue) {
  ...
  # Convert to dense matrix for Rcpp (faster than sparse for this operation)
  st_data_dense <- as.matrix(st_data)

  # Get gene indices (1-based for R)
  ligand_idx <- match(lrdb$ligand, rownames(st_data))
  receptor_idx <- match(lrdb$receptor, rownames(st_data))

  # Get cell indices (1-based)
  sender_idx <- match(cell_pair$cell_sender, colnames(st_data))
  receiver_idx <- match(cell_pair$cell_receiver, colnames(st_data))

  # Run permutation test using C++ implementation
  tryCatch(
    {
      perm_result <- cpp_permutation_test(
        st_data_dense,
        ligand_idx,
        receptor_idx,
        sender_idx,
      ...
      lrdb$lr_co_ratio <- perm_result$real_ratios
      lrdb$lr_co_exp_num <- lrdb$lr_co_ratio * nrow(cell_pair)
      lrdb$lr_co_ratio_pvalue <- perm_result$pvalues
    },
    error = function(e) {
      # Fallback to R implementation
      warning("C++ permutation test failed, using R fallback")
      ndata_ligand <- st_data[lrdb$ligand, cell_pair$cell_sender, drop = FALSE]
      ndata_receptor <- st_data[lrdb$receptor, cell_pair$cell_receiver, drop = FALSE]

      if (nrow(lrdb) == 1) {
        lr_coexp <- sum((ndata_ligand > 0) & (ndata_receptor > 0))
        lrdb$lr_co_exp_num <<- lr_coexp
        lrdb$lr_co_ratio <<- lr_coexp / ncol(ndata_ligand)
      } else {
        lrdb$lr_co_ratio <<- .co_exp_matrix(ndata_ligand, ndata_receptor)
        lrdb$lr_co_exp_num <<- lrdb$lr_co_ratio * nrow(cell_pair)
      }
      lrdb$lr_co_ratio_pvalue <<- 1 # Set to 1 as fallback
    }
  )
...
.generate_tf_res <- function(tf_gene_all, celltype_sender, celltype_receiver, receptor_name, ggi_res) {
  receptor_tf_temp <- data.frame(
    celltype_sender = celltype_sender, celltype_receiver = celltype_receiver,
...
.get_tf_res_doParallel <- function(celltype_sender, celltype_receiver, lrdb, ggi_tf, cell_pair, st_data, max_hop, co_exp_ratio) {
  ...
  receptor_tf <- NULL
  receptor_tf <- foreach::foreach(
    j = 1:length(receptor_name), .packages = "Matrix", .combine = "rbind",
    .export = c(".generate_ggi_res", ".generate_tf_gene_all", ".generate_tf_res", ".random_walk")
  ) %dopar% {
    # generate ggi_res
...
.get_tf_res <- function(celltype_sender, celltype_receiver, lrdb, ggi_tf, cell_pair, st_data, max_hop, co_exp_ratio) {
  ...
  # Random walk algorithm for TF scoring
  .random_walk <- function(receptor_tf, ggi_res) {
    ...
    adj_list <- split(ggi_res, ggi_res$src)
    set.seed(123)

    for (i in 1:n_walks) {
      current_node <- receptor_name
      n_hop <- 0

      while (n_hop < max_n_hop) {
        if (!current_node %in% names(adj_list)) break

        neighbors <- adj_list[[current_node]]
        if (nrow(neighbors) == 0) break

        next_idx <- sample.int(nrow(neighbors), 1)
        next_node <- neighbors$dest[next_idx]

        if (next_node %in% names(tf_score)) {
          tf_score[next_node] <- tf_score[next_node] + 1
        }
    ...
    return(as.numeric(tf_score / n_walks))
  }
  receptor_tf <- NULL
  receptor_name <- unique(lrdb$receptor)
...
# Random walk for TF scoring
.random_walk <- function(receptor_tf, ggi_res) {
  receptor_name <- unique(receptor_tf$receptor)
  tf_names <- receptor_tf$tf

  # Use C++ implementation with R fallback
  tryCatch(
    {
      tf_scores <- cpp_random_walk(
        ggi_src = as.character(ggi_res$src),
        ggi_dest = as.character(ggi_res$dest),
        receptor_name = as.character(receptor_name),
        tf_names = as.character(tf_names),
        n_walks = 10000,
        max_hop = 10,
        seed = 123
      )
      return(as.numeric(tf_scores))
    },
    error = function(e) {
      # Fallback to R implementation
      warning("C++ random walk failed, using R fallback")
      ...
      return(as.numeric(tf_score / 10000))
    }
  )
}
...
#'
#' @param object A SpaTalk object
#' @return Invisible NULL, prints summary information
#' @import Matrix
