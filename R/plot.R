...
plot_st_pie_generate <- function(st_meta, pie_scale = 1, xy_ratio = 1, color = NULL) {
  ...
  ggplot2::ggplot() +
    scatterpie::geom_scatterpie(
      data = st_meta, ggplot2::aes(x = x, y = y), col = cellname,
      color = NA, pie_scale = pie_scale
    ) +
    ggplot2::coord_fixed(ratio = 1) +
    ggplot2::scale_fill_manual(values = col_manual) +
    ggplot2::theme_bw() +
...
plot_st_gene <- function(
    object, gene, size = 1, color_low = "grey", color_mid = NULL, color_high = "blue",
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_point(ggplot2::aes(st_meta$x, st_meta$y, color = st_meta$gene),
      size = size
    ) +
    ggplot2::theme_bw() +
    ggplot2::theme(panel.grid = ggplot2::element_blank()) +
...
plot_st_celltype <- function(object, celltype, size = 1, color_celltype = "blue", color_others = "grey") {
  ...
  ggplot2::ggplot() +
    ggplot2::geom_point(ggplot2::aes(st_meta$x, st_meta$y, color = st_meta$celltype),
      size = size
    ) +
    ggplot2::scale_color_manual(values = c(color_celltype, color_others)) +
    ggplot2::theme_bw() +
    ggplot2::theme(panel.grid = ggplot2::element_blank()) +
...
plot_st_celltype_density <- function(
    object, celltype, type, if_plot_point = T, point_color = NULL, point_size = 1,
...
plot_st_celltype_percent <- function(
    object, celltype, size = 1, color_low = NULL, color_mid = NULL, color_high = NULL,
    color_midpoint = NULL) {
  # check
  if (!is(object, "SpaTalk")) {
    stop("Invalid class for object: must be 'SpaTalk'!")
  ...
  if (is.null(color_mid)) {
    ggplot2::ggplot() +
      ggplot2::geom_point(ggplot2::aes(st_meta$x, st_meta$y, color = st_meta$celltype_percent),
        size = size
      ) +
...
plot_st_celltype_all <- function(object, size = 1, color = NULL) {
  ...
  ggplot2::ggplot() +
    ...
    ggplot2::scale_color_manual(values = col_manual) +
    ggplot2::theme_bw() +
    ggplot2::theme(panel.grid = ggplot2::element_blank()) +
    ggplot2::labs(x = "x", y = "y", color = "celltype")
}
...
plot_st_cor_heatmap <- function(
    object, marker_genes, celltypes, color_low = NULL, color_mid = NULL, color_high = NULL,
  ...
  if (if_show_top) {
    ...
    pheatmap::pheatmap(
      mat = plot_cor, cluster_rows = F, cluster_cols = F, border_color = border_color,
...
plot_ccdist <- function(
    object, celltype_sender, celltype_receiver, color = NULL, size = 1, if_plot_others = T,
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_point(
      data = st_meta, ggplot2::aes(x, y, color = celltype),
      size = size
    ) +
...
plot_cci_lrpairs <- function(
    object, celltype_sender, celltype_receiver, top_lrpairs = 20, color = NULL, border_color = "black", type = "sig",
  ...
  if (type == "sig") {
    ...
    pheatmap::pheatmap(lrpair_mat,
      cluster_cols = F, cluster_rows = F, color = heat_col, border_color = border_color, legend = F, display_numbers = plot_res,
...
plot_lrpair <- function(
    object, celltype_sender, celltype_receiver, ligand, receptor, color = NULL, size = 1,
    if_plot_density = T, if_plot_edge = T, if_show_arrow = T, arrow_length = 0.05, plot_cells = NULL) {
  # check
  if (!is(object, "SpaTalk")) {
    stop("Invalid class for object: must be 'SpaTalk'!")
  ...
  p <- ggplot2::ggplot() +
    ggplot2::geom_point(
      data = st_meta, ggplot2::aes(x, y, color = Expressed_genes),
      size = size
    ) +
    ggplot2::scale_color_manual(values = color) +
    ggplot2::theme_bw() +
...
plot_lrpair_vln <- function(
    object, celltype_sender, celltype_receiver, ligand, receptor, vln_color = NULL,
...
plot_lr_path <- function(
    object, celltype_sender, celltype_receiver, ligand, receptor, color = NULL, size = 5,
    arrow_length = 0.1) {
  # check
  if (!is(object, "SpaTalk")) {
    stop("Invalid class for object: must be 'SpaTalk'!")
  ...
  plot_node_new$Celltype <- factor(plot_node_new$Celltype, levels = c(celltype_sender, celltype_receiver))
  ggplot2::ggplot() +
    geom_segment(data = tf_path_all, aes(x = src_x, y = src_y, xend = dest_x, yend = dest_y)) +
    ggplot2::geom_point(data = plot_node_new, ggplot2::aes(x, y, color = Celltype), size = size) +
    ggplot2::scale_color_manual(values = col_manual) +
    ggrepel::geom_label_repel(
      data = plot_node_new,
      aes(x, y, label = gene, fill = tf)
    ) +
    labs(x = NULL, y = NULL) +
    theme(
...
plot_path2gene <- function(
    object, celltype_sender, celltype_receiver, ligand, receptor, min_gene_num = 5,
  ...
  p <- ggplot(gene2pathway_plot, aes(
    x = cluster, y = num, stratum = type, fill = cluster, alluvium = group,
    label = type
  )) +
    ggalluvial::geom_stratum(alpha = 1, width = 0.1) +
    ...
    theme(
      axis.text.y.left = element_blank(), panel.grid = element_blank(), axis.ticks = element_blank(),
